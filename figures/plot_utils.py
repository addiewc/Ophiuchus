...
def make_conventional_plot(model_scores: Dict[str, Tuple[float, float]], restrict_to_models = None, max_range = 10, alpha=1):
    ...
    edge_colors = [plot_settings.get_model_edgecolor(m) for m in model_ordering]
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_xlabel(plot_settings.get_metric_name("economic_score"), loc="right", fontsize=12)
    ax.set_ylabel(plot_settings.get_metric_name("social_score"), loc="top", fontsize=12)
    ax.spines['left'].set_position('center')
    ax.spines['bottom'].set_position('center')
    format_ax(ax)
    
    for marker in "o", "P":  # plot one marker at a time
        ax.scatter(
            x=[model_scores[model_ordering[i]][0] for i in range(len(model_ordering)) if markers[i] == marker],
            y=[model_scores[model_ordering[i]][1] for i in range(len(model_ordering)) if markers[i] == marker],
            color=[colors[i] for i in range(len(model_ordering)) if markers[i] == marker],
            marker=marker,
